...
//TODO trader can currently send only one Trade_Message i.e a trader can only trade one stock per tick right now. Change this later.
...
public:
    ...
    Trader(std::string id,
        ...
        : trader_id(std::move(id)), 
          trade_func(std::move(func)),
          cash_balance(std::max(initial_cash, 0.0)),
          stock_holdings()
    {}
    ...
    void updateStockHoldings(const std::string& ticker, int quantity) {
      ...
      if (stock_holdings[ticker] <= 0) {stock_holdings.erase(ticker);} 
    }
    
    //Expects +ive input for gaining cash, -ive input for losing cash
    void updateCashBalance(double quantity){
