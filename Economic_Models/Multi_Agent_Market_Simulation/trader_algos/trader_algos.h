...
auto random_trader =
    [](const MarketDataframe &market) -> Trades::Trade_Message {
  ...
  std::uniform_int_distribution num_stocks{0, 10};

  int upper = static_cast<int>(latest_frame.curr_price * 1.2);
  int lower = static_cast<int>(latest_frame.curr_price * 0.9);
  lower = 10;
  if (upper < lower)
    std::swap(upper, lower); // ensure valid range
  ...
  return {std::string(latest_frame.ticker),
          static_cast<Trades::TradeType>(order_type(Random::mt)),
          static_cast<double>(stock_price(Random::mt)), num_stocks(Random::mt)};
};

auto market_sell_trader =
    [](const MarketDataframe &market) -> Trades::Trade_Message {
  // Picks first stock randomly and always buys 10 shares at current price
  if (market.sf_ll.empty()) {
    return {"", static_cast<Trades::TradeType>(0), 0.0, 0}; // no-op
  ...
  return {std::string(latest_frame.ticker), Trades::MARKET_SELL, 0, 10};
};

auto market_buy_trader =
    [](const MarketDataframe &market) -> Trades::Trade_Message {
  // Picks first stock randomly and always buys 10 shares at current price
  if (market.sf_ll.empty()) {
    return {"", static_cast<Trades::TradeType>(0), 0.0, 0}; // no-op
  ...
  return {std::string(latest_frame.ticker), Trades::MARKET_BUY, 0, 10};
};

auto limit_buy_trader =
    [](const MarketDataframe &market) -> Trades::Trade_Message {
  // Picks first stock randomly and always buys 10 shares at current price
  if (market.sf_ll.empty()) {
    return {"", static_cast<Trades::TradeType>(0), 0.0, 0}; // no-op
  ...
  return {std::string(latest_frame.ticker), Trades::LIMIT_BUY, 40, 1};
};
